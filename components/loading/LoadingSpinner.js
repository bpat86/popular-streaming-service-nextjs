import { useContext } from "react";
import { motion } from "framer-motion";
// Context
import ProfileContext from "@/context/ProfileContext";

const LoadingSpinner = () => {
  const { activeProfile } = useContext(ProfileContext);
  const userAvatar = `url("/images/profiles/avatars/${activeProfile.attributes.avatar}.png")`;
  const loadingSpinnerURL =
    "data:image/svg+xml;base64,";

  return activeProfile ? (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="flex items-center justify-center absolute inset-0 w-full overflow-hidden"
    >
      <div
        className="profile-avatar w-12 h-12 bg-cover absolute rounded-md z-10"
        style={{
          backgroundImage: userAvatar,
        }}
      />
      <img
        src={loadingSpinnerURL}
        className="motion-safe:animate-spin absolute w-[10.5rem] h-[10.5rem] text-netflix-red fill-netflix-red"
      />
    </motion.div>
  ) : (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="flex items-center justify-center absolute inset-0 w-full overflow-hidden"
    >
      <img
        src={loadingSpinnerURL}
        className="motion-safe:animate-spin absolute w-24 h-24 text-netflix-red fill-netflix-red"
      />
    </motion.div>
  );
};

export default LoadingSpinner;

...
const PreviewModal = forwardRef((props, layoutWrapperRef) => {
   ...
   * @param {Object} query
   */
  const updateRoute = ({ id, mediaType } = {}) => {
    router.push(
      {
  ...
  const getDetailModalAnimationProps = () => {
    ...
    return {
      ...
      onAnimationComplete: (currentAnimationState) => {
        if (currentAnimationState === animationStateActions.MOUNT_DETAIL_MODAL)
          return (
            window.scrollTo(0, 0),
            flushSync(() =>
              setAnimationState(animationStateActions.OPEN_DETAIL_MODAL)
            ),
            currentAnimationState === animationStateActions.OPEN_DETAIL_MODAL &&
